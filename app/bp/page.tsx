...
async function dbGet(col: string, email: string) {
  const r = await fetch(`/api/db?col=${col}`, { headers: { "x-user-email": email } });
  return r.json();
}
async function dbPut(col: string, data: any, email: string) {
...
function classify(r: { sys: number; dia: number; pulse: number }) {
  const out: { level: "ok" | "warn" | "alert"; msg: string }[] = [];
  if (r.pulse > 100)
    out.push({ level: "alert", msg: `心率偏高（${r.pulse}）。先静坐休息 5 分钟再量一次；若持续 >100，或伴胸闷/胸痛/头晕/喘不过气 → 尽快就医。` });
  else if (r.pulse < 60)
    out.push({ level: "warn", msg: `心率偏低（${r.pulse}）。若伴头晕/无力请就医（她在吃减慢心率的药）。` });
  if (r.sys >= 140 || r.dia >= 90)
    out.push({ level: "warn", msg: `血压偏高（${r.sys}/${r.dia}）。留意并记录；若 ≥180/110 或头痛/胸闷 → 就医。` });
  else if (r.sys < 90 || r.dia < 60)
    out.push({ level: "warn", msg: `血压偏低（${r.sys}/${r.dia}）。起身慢一点、注意补水，若头晕请小心。` });
  return out;
}
...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      const saved = typeof window !== "undefined" ? localStorage.getItem("bp-email") || "" : "";
      if (saved) {
        const f = await dbGet("family", saved);
        if (f.member) {
          const me = (f.list as Member[]).find((m) => m.email === saved);
          if (me) { setUser(me); await loadAll(saved); }
        }
      }
      setLoading(false);
    })();
  ...
  async function loadAll(email: string) {
    const [rd, rm, f] = await Promise.all([
      dbGet("readings", email), dbGet("reminders", email), dbGet("family", email),
    ]);
    ...
    if (rems.length === 0) {
      ...
      await dbPut("reminders", rems, email);
    }
    setReminders(rems);
  ...
  async function checkLogin() {
    const email = norm(lEmail);
    if (!email) return;
    const f = await dbGet("family", email);
    if (f.setup) { setSetupMode(true); return; }
    if (f.member) {
      const me = (f.list as Member[]).find((m) => m.email === email)!;
      localStorage.setItem("bp-email", email);
      setUser(me); await loadAll(email); setLoginErr("");
    } else setLoginErr("这个 Email 不在家人名单内。请让管理员把你加入。");
  }
  async function doSetup() {
    const email = norm(lEmail);
    if (!email || !lName.trim()) return;
    const me: Member = { email, name: lName.trim(), isAdmin: true };
    await dbPut("family", [me], email);
    // seed today's 3 real readings once
    ...
    localStorage.setItem("bp-email", email);
    setUser(me); await loadAll(email);
  }
  function logout() { localStorage.removeItem("bp-email"); setUser(null); setLEmail(""); setSetupMode(false); }
  ...
  useEffect(() => {
    ...
    if ("Notification" in window && Notification.permission === "default") {
      // ask lazily; non-blocking
    }
    const iv = setInterval(() => {
      const hm = nowTime();
      const stamp = todayStr() + " " + hm;
      ...
      if (due) {
        ...
        if ("Notification" in window && Notification.permission === "granted")
          new Notification("妈妈健康提醒", { body: msg });
        setToast(msg);
  ...
  if (loading) return <div className="min-h-screen grid place-items-center text-teal-700">…</div>;
    ...
    return (
      <div className="min-h-screen bg-teal-50 grid place-items-center p-4">
        <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-6">
          ...
          <p className="text-gray-500 text-sm mb-5">家庭共享 · 记录 · 运动 · 提醒</p>
          <h2 className="font-semibold text-gray-700 mb-2">{setupMode ? "首次设置（你将成为管理员）" : "家人登录"}</h2>
          <input className="w-full border rounded-lg px-3 py-2 mb-3" placeholder="你的 Email"
            value={lEmail} onChange={(e) => setLEmail(e.target.value)} />
          {setupMode && (
            <input className="w-full border rounded-lg px-3 py-2 mb-3" placeholder="你的名字"
              value={lName} onChange={(e) => setLName(e.target.value)} />
          )}
          {loginErr && <p className="text-red-600 text-sm mb-3">{loginErr}</p>}
          <button onClick={setupMode ? doSetup : checkLogin}
            className="w-full bg-teal-700 text-white rounded-lg py-2.5 font-medium hover:bg-teal-800">
            {setupMode ? "创建并成为管理员" : "进入"}
          </button>
          <p className="text-xs text-gray-400 mt-4 leading-relaxed">
            提示：此「登录」是家人访问名单，非严格加密。数据存于 Upstash，供家人内部使用。
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-2xl mx-auto p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">{user.name}{user.isAdmin ? " · 管理员" : ""}</p>
          </div>
          <button onClick={logout} className="text-xs text-gray-500 underline">登出</button>
        </div>
        ...
        <div className="mt-4">
          {view === "add" && <AddView user={user} readings={readings} setReadings={setReadings} />}
          {view === "history" && <HistoryView readings={readings} setReadings={setReadings} email={user.email} />}
          {view === "chart" && <ChartView readings={readings} />}
          {view === "report" && <ReportView readings={readings} />}
          {view === "exercise" && <Exercises />}
          {view === "reminder" && <ReminderView reminders={reminders} setReminders={setReminders} email={user.email} />}
          {view === "settings" && <SettingsView user={user} family={family} setFamily={setFamily} />}
        </div>
...
function H({ children }: { children: React.ReactNode }) {
  return <h2 className="font-semibold text-teal-800 mb-3">{children}</h2>;
}

function AddView({ user, readings, setReadings }: any) {
  ...
  async function save() {
    ...
    const r: Reading = { id: Date.now(), date: d, time: tm, sys: +sys, dia: +dia, pulse: +pulse, by: user.name, note: note.trim() };
    const next = [...readings, r];
    setReadings(next); await dbPut("readings", next, user.email);
  ...
  return (
    <Card>
      ...
      {preview.map((p, i) => (
        <div key={i} className={`mt-3 rounded-lg p-3 text-sm ${p.level === "alert" ? "bg-red-50 text-red-700 border border-red-200" : "bg-orange-50 text-orange-700 border border-orange-200"}`}>
          ⚠️ {p.msg}
        </div>
      ))}

      <p className="text-xs text-gray-400 mt-3">参考范围：高压 90–129 · 低压 60–79 · 心率 60–100 /min</p>
      <button onClick={save} className="w-full mt-4 bg-teal-700 text-white rounded-lg py-2.5 font-medium hover:bg-teal-800">
        {flash ? "已保存 ✓" : "保存记录"}
      </button>
...
function HistoryView({ readings, setReadings, email }: any) {
  ...
  async function del(id: number) {
    ...
    setReadings(next); await dbPut("readings", next, email);
  }
  return (
    <Card>
      <H>历史记录</H>
...
function ReminderView({ reminders, setReminders, email }: any) {
  ...
  async function save(next: Reminder[]) { setReminders(next); await dbPut("reminders", next, email); }
  return (
    <Card>
      <H>提醒（App 开着时到点提醒）</H>
      {canNotify && perm !== "granted" && (
        <button onClick={async () => setPerm(await Notification.requestPermission())}
          className="w-full mb-3 border border-teal-700 text-teal-700 rounded-lg py-2 text-sm font-medium">
          开启浏览器通知
        </button>
      )}
      <div className="space-y-2">
      ...
      <p className="text-xs text-gray-400 mt-3 leading-relaxed">
        注意：网页只能在「App 开着」时到点提醒。要 App 关着也收到（像闹钟），最可靠的是<b>手机自带闹钟</b>，
        或之后接 Telegram / WhatsApp 机器人定时推送。
      </p>
    </Card>
  );
}

function SettingsView({ user, family, setFamily }: any) {
  ...
  async function add() {
    ...
    setFamily(next); await dbPut("family", next, user.email); setNewEmail("");
  }
  async function remove(e: string) {
    const next = family.filter((m: Member) => m.email !== e);
    setFamily(next); await dbPut("family", next, user.email);
  }
  return (
    <Card>
      <H>设置 · 家人名单</H>
      <div className="space-y-2">
        {family.map((m: Member) => (
          <div key={m.email} className="flex items-center justify-between border rounded-lg px-3 py-2 text-sm">
            <span>{m.email}{m.isAdmin && <span className="text-teal-700 text-xs"> · 管理员</span>}{m.email === user.email && <span className="text-gray-400 text-xs"> (你)</span>}</span>
            {user.isAdmin && !m.isAdmin && <button onClick={() => remove(m.email)} className="text-xs text-gray-400 hover:text-red-500">移除</button>}
          </div>
        ))}
      </div>
      {user.isAdmin ? (
      ...
      <p className="text-xs text-gray-400 mt-4 leading-relaxed">登录是家人访问名单，非严格加密。数据存于 Upstash。</p>
    </Card>
  );
}
